...
with open('reportQuast_mis_paramSensi_sort.txt') as f :
	for line in f :
...
with open('reportQuast_mis_init_sort.txt') as f :
	for line in f :
			if re.match("^15X", line) is not None :
				a,b=line.split("\t")
				c=float(b)
...
plt.axhline(y=0, color='lightgreen', linestyle='-', linewidth=0.7)
...
plt.savefig('boxplot_quast_mis.png', dpi=200)
plt.show()
